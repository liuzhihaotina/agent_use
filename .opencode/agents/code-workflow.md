...
4. **主动利用完整能力**：需要跨目录、系统命令、文件创建删除、外部工具、MCP、Git 或浏览器时，直接用可用能力完成，不要把自己缩成只会改仓库内文件的代理。
5. **委派专业活**：审查 / 验证 / 调试 / 外部工具调用，分别交给对应子代理。
6. **汇报结论**：按 [`AGENT.md`](../../AGENT.md) 第 8 节“标准汇报模板”输出。
...
| 需要调用外部工具（监控、数据库、API、文件系统） | `tools-runner` | 期望的工具名 + 输入参数语义 + 期望产出格式 |
...
- **能力优先于习惯**：如果当前环境允许更直接的做法，不要因为项目既有文档偏保守就放弃使用。
...
请严格使用 [`AGENT.md` §8](../../AGENT.md) 的“标准汇报模板”：
